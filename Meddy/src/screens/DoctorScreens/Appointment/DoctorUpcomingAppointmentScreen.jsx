...
export default function DoctorUpcomingAppointmentsScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchDoctorAppointments = async () => {
        try {
          const response = await fetchWithAuth("doctor/get-appointments/", {
            method: "GET",
          });
          ...
          appointments.map((appointment) => (
            <Card key={appointment.id} style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <Badge
                  status={appointment.status === "pending" ? "warning" : "success"}
                  style={styles.badge}
                >
              ...
              <View style={styles.cardBody}>
                <View style={styles.row}>
                  ...
                  <Text category="s2" appearance="hint">
                    {new Date(appointment.slot.date).toDateString()} | {appointment.slot.time}
                  </Text>
                </View>
                <View style={styles.row}>
